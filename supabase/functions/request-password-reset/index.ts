  ...
  try {
    ...
    if (profile) {
      ...
      const { error: emailError } = await resend.emails.send({
        from: "FinaManage <onboarding@resend.dev>",
        to: [email],
        subject: "FinaManage — Redefinição de senha",
        html: `
          <!DOCTYPE html>
          <html>
            <head>
            ...
            <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
              <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
                <h1 style="color: white; margin: 0; font-size: 28px;">FinaManage</h1>
              </div>
              
              <div style="background: #ffffff; padding: 40px 30px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 10px 10px;">
                <h2 style="color: #333; margin-top: 0;">Olá${profile.nome ? `, ${profile.nome}` : ''}!</h2>
                
                <p>Recebemos uma solicitação para redefinir a senha da sua conta FinaManage.</p>
                ...
                <p style="margin-top: 30px; font-size: 14px; color: #999;">
                  Atenciosamente,<br>
                  <strong>Equipe FinaManage</strong>
                </p>
              </div>
              
              <div style="text-align: center; padding: 20px; font-size: 12px; color: #999;">
                <p>© ${new Date().getFullYear()} FinaManage. Todos os direitos reservados.</p>
              </div>
            </body>
          </html>
        `,
